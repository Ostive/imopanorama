      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          actions={
            <Link href="/admin/faqs/new">
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center px-6 py-2 bg-linear-to-r from-primary-600 to-blue-600 hover:from-primary-700 hover:to-blue-700 text-white font-semibold rounded-xl transition-all shadow-lg"
              >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl shadow-lg overflow-hidden">
          {loading ? (
            ...
            <div className="px-6 py-12 text-center text-red-600">Erreur lors du chargement des questions.</div>
          ) : faqs.length === 0 ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {faqs.map((faq) => (
                    <tr key={faq.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm font-semibold text-gray-700">{faq.order}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {faq.isActive ? (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="hidden md:flex items-center justify-end gap-2">
                          <Link href="/faqs" target="_blank" className="p-2 text-primary-600 hover:text-primary-900 hover:bg-primary-50 rounded-lg transition-colors" title="Voir">
                            <EyeIcon className="h-5 w-5" />
                          </Link>
                          <Link href={`/admin/faqs/${faq.id}/edit`} className="p-2 text-blue-600 hover:text-blue-900 hover:bg-blue-50 rounded-lg transition-colors" title="Modifier">
                            <PencilIcon className="h-5 w-5" />
                          </Link>
                          <button onClick={() => handleDelete(faq)} className="p-2 text-red-600 hover:text-red-900 hover:bg-red-50 rounded-lg transition-colors" title="Supprimer">
                        ...
                        <div className="md:hidden flex justify-end">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem asChild>
                                <Link href={`/admin/faqs/${faq.id}/edit`} className="flex items-center gap-2 text-blue-600">
