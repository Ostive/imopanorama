...
function ContactsList({ contacts, onOpen }: ContactsListProps) {
  return (
...
interface ContactDetailModalProps {
  ...
  onLocalEdit: (updates: Partial<Contact>) => void;
  onMarkRead: (id: string) => void;
  onRequestDelete: (id: string) => void;
}

function ContactDetailModal({ contact: c, saving, onClose, onUpdate, onLocalEdit, onMarkRead, onRequestDelete }: ContactDetailModalProps) {
  return (
    <div
      aria-hidden="true"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 sm:px-8 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3 mb-4">
              ...
              {saving && <span className="text-xs font-semibold text-primary-600 dark:text-primary-400">Sauvegarde...</span>}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(event) => onUpdate({ leadStatus: event.target.value as LeadStatus })}
              ...
              <label className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(event) => onUpdate({ leadPriority: event.target.value as LeadPriority })}
                  className="w-full h-10 rounded-lg border border-border bg-card px-3 text-sm text-foreground"
                >
              ...
              <label className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(event) => onUpdate({ nextFollowUpAt: event.target.value ? new Date(event.target.value).toISOString() : null })}
                  className="w-full h-10 rounded-lg border border-border bg-card px-3 text-sm text-foreground"
                />
              </label>
              <label className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(event) => onUpdate({ scheduledVisitAt: event.target.value ? new Date(event.target.value).toISOString() : null })}
                  className="w-full h-10 rounded-lg border border-border bg-card px-3 text-sm text-foreground"
                />
              </label>
            </div>
            <div className="mt-4 grid gap-4">
              <label className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(event) => onLocalEdit({ visitOutcome: event.target.value })}
                  onBlur={(event) => onUpdate({ visitOutcome: event.target.value || null })}
              ...
              <label className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={(event) => onLocalEdit({ internalNotes: event.target.value })}
                  onBlur={(event) => onUpdate({ internalNotes: event.target.value || null })}
                  rows={3}
...
function ContactsPageContent() {
  ...
  const [listParams, updateListParams] = useMergedState(() => ({
    search: urlParams.get('search') || '',
    statuses: urlParams.get('status') ? [urlParams.get('status') as string] : ([] as string[]),
    page: parseInt(urlParams.get('page') || '1'),
    limit: parseInt(urlParams.get('limit') || '15'),
  }));
  const { search: searchTerm, statuses: selectedStatuses, page: currentPage, limit } = listParams;
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-2xl shadow-lg p-6 mb-6 border border-border">
          <div className="flex items-center justify-between mb-4">
            ...
            {(searchTerm || selectedStatuses.length > 0) && (
              <button type="button" onClick={() => { updateListParams({ search: '', statuses: [], page: 1 }); replaceUrl({ search: '', status: '', page: 1 }); }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <input type="text" value={searchTerm} onChange={(e) => { updateListParams({ search: e.target.value, page: 1 }); replaceUrl({ search: e.target.value, page: 1 }); }} placeholder="Nom, email, terrain..." aria-label="Rechercher un contact"
              ...
              {searchTerm && (
                <button type="button" onClick={() => { updateListParams({ search: '', page: 1 }); replaceUrl({ search: '', page: 1 }); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-gray-600 dark:hover:text-gray-300">
            ...
            <CheckboxDropdown
              ...
              onChange={(values) => { updateListParams({ statuses: values, page: 1 }); replaceUrl({ status: values.length === 1 ? values[0] : '', page: 1 }); }}
              options={[
        ...
        <div className="bg-card rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-black/30 border border-border overflow-hidden">
          ...
          <AdminTablePagination
            ...
            onPageChange={(nextPage) => { updateListParams({ page: nextPage }); replaceUrl({ page: nextPage }); }}
            onLimitChange={(nextLimit) => { updateListParams({ limit: nextLimit, page: 1 }); replaceUrl({ limit: nextLimit, page: 1 }); }}
          />
        </div>
        ...
        {isModalOpen && selectedContact && (
          <ContactDetailModal
            contact={contacts.find((x) => x.id === selectedContact.id) || selectedContact}
            saving={savingCrm}
            onClose={() => setIsModalOpen(false)}
            onUpdate={updateSelectedContact}
            onLocalEdit={(updates) => setSelectedContact((prev) => (prev ? { ...prev, ...updates } : prev))}
            onMarkRead={markAsRead}
            onRequestDelete={(id) => { contactToDeleteRef.current = id; setDeleteModalOpen(true); }}
          />
        )}
