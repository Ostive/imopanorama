...
const COLOR_MAP: Record<string, { border: string; bg: string; text: string }> = {
  '#3b82f6': { border: 'border-blue-500', bg: 'bg-blue-500/10', text: 'text-blue-600' },
...
export function StatsCard({ title, value, subtitle, icon, color, delay = 0, trend, invertTrendColor }: StatsCardProps) {
  const c = COLOR_MAP[color] || { border: 'border-blue-500', bg: 'bg-blue-500/10', text: 'text-blue-600' };
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      whileHover={{ y: -4 }}
      className={`bg-white rounded-2xl shadow-lg hover:shadow-xl p-6 border-l-4 ${c.border} transition-shadow`}
    >
