...
export default function Component({
  ...
  const renderPaginationDots = () => {
    ...
    let startDot = Math.max(0, Math.min(currentSlide - 2, totalImages - totalDots))
    ...
    return Array.from({ length: endDot - startDot }, (_, i) => i + startDot).map((index) => (
      <button
        key={index}
  ...
  return (
    ...
    >
      <div className="relative">
        <Carousel className="w-[300px] h-[300px] rounded-lg overflow-hidden" setApi={setApi}>
          <CarouselContent>
      ...
      <CardContent className="py-2 pr-4 pl-0">
        <h3 className="text-xl font-bold text-primary mb-1 ml-0">{price}</h3>
